...
def generalProcess(fileName, patchFolder, objectName):
    ...
    if (inletTildeNumber > 1) or (outletTildeNumber > 1):
        ...
        #object 4 is the outlet~
        outletTildeId = 4
        pdCodeMC.append("#X obj 100 230 outlet~;\n")
        #if there is at least one inlet~, we create the object next to the outlet~~
        if (inletTildeNumber > 0):
            inletTildeId = outletTildeId + 1
            pdCodeMC.append("#X obj 100 30 inlet~;\n")
        else:
            inletTildeId = -1
        #if there are more than one inlet~, we need a snake~ out object to group the inlet~
        if (inletTildeNumber > 1):
            #there is an inlet~ as (more than 1)
            snakeOutId = inletTildeId + 1
            pdCodeMC.append("#X obj 100 80 snake~ out "+str(inletTildeNumber)+";\n")
        else:
            snakeOutId = -1
        #if there are more than one outlets from the faust objet, we need a snake~ in object
        if (outletTildeNumber > 1):
            if (inletTildeNumber == 0):
                snakeInId = outletTildeId + 1
            else:
                if (inletTildeNumber == 1):
                    snakeInId = inletTildeId + 1
                else:
                    snakeInId = inletTildeId + 2
        ...
        #if there is only one output, direction connection to the outlet~
        if (outletTildeNumber == 1):
            pdCodeMC.append("#X connect "+str(faustObjectId)+" 1 "+str(outletTildeId)+" 0;\n")
        else:
            if (outletTildeNumber > 1):
                #connects outputs of the faust compiled object to the inputs of the snake~ in
                for i in range(outletTildeNumber):
                    pdCodeMC.append("#X connect "+str(faustObjectId)+" "+str(i+1)+" "+str(snakeInId)+" "+str(i)+";\n")
                #connects the snake~ in to the outlet~~
                pdCodeMC.append("#X connect "+str(snakeInId)+" 0 "+str(outletTildeId)+" 0;\n")
            else:
                print("error, no outlets ???\n")
        #connections on the inlet~ side
        if (inletTildeNumber == 1):
            #connects the only inlet~ to the faust compiled object (input #1)
            pdCodeMC.append("#X connect "+str(inletTildeId)+" 0 "+str(faustObjectId)+" 1;\n")
        else:
            if (inletTildeNumber > 1):
                #connects the inlet~ to the snake~ out
                pdCodeMC.append("#X connect "+str(inletTildeId)+" 0 "+str(snakeOutId)+" 0;\n")
                #connects the outputs ot the snake~ out to the inputs of the faust compiled object
                for i in range(inletTildeNumber):
                    pdCodeMC.append("#X connect "+str(snakeOutId)+" "+str(i)+" "+str(faustObjectId)+" "+str(i+1)+";\n")
